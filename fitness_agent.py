...
class FitnessAgent:
    def __init__(self, openai_api_key: str, nut_api_key: str):
        ...
        self.agent = Agent(
            ...
            functions=[
                ...
                self.get_workout_info,
                calculate_bmr,
                calculate_tdee,
                calculate_ibw,
                calculate_bmi,
                calculate_calories_to_lose_weight,
            ],
        )
    ...
    def get_nutritional_info(self, query: str) -> dict:
        ...
        if response.status_code == requests.codes.ok:
            return (
                response.json()
            )  # Use json instead of text for a more structured data
        else:
            return {"Error": response.status_code, "Message": response.text}

    def get_workout_info(
        self, name: str = "", type: str = "", muscle: str = "", difficulty: str = ""
    ) -> dict:
        """Fetches the workout information for a specific workout item

        :param name: The name of the workout item
        :param type: The type of the workout item
        :param muscle: The muscle of the workout item
        :param difficulty: The difficulty of the workout item
        :return: The workout information of the workout item
        """
        api_url = (
            "https://api.api-ninjas.com/v1/exercises?name={}&".format(name)
            + "type={}&".format(type)
